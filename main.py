...
def suggest_symptoms(user_input):
    ...
    suggestions = process.extract(user_input, choices, limit=5)
    return [symptom for symptom, score in suggestions]
...
def home():
    if request.method == 'POST':
        ...
        while True:
            ...
            if invalid_symptoms:
                suggestions = [suggest_symptoms(symptom) for symptom in invalid_symptoms]
                message = "Invalid symptoms detected. Suggestions: " + ", ".join([", ".join(s) for s in suggestions])
